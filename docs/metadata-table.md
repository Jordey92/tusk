...
- `id` must be auto-generated by the database, using `SERIAL` or an equivalent
  identity/default sequence.
- `executed_at` must default to `now()`.

Tusk treats unexpected columns, missing required columns, wrong column types,
missing required defaults, missing auto-generation, or missing required
constraints as an invalid metadata table shape.
